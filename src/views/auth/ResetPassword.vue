    ...
    <div class="flex h-full w-full flex-col items-center justify-center gap-4">
      ...
      >
        <div class="flex">
          ...
          <div>
            <h3 class="font-semibold">{{ tokenError ? 'Token Error' : 'Validation Error' }}</h3>
            <p class="mt-1 text-sm">{{ error }}</p>
          </div>
        </div>
      </div>

      <div v-if="!token" class="w-full rounded-lg border border-destructive/50 bg-destructive/10 p-4 text-destructive">
        <div class="flex">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            width="20"
      ...
      <form v-else @submit.prevent="handleSubmit" class="grid w-full gap-3.5">
